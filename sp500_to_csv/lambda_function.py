...
SP500_URL = os.environ.get("SP500_URL")
...
def create_sp500_df(soup: BeautifulSoup) -> pd.DataFrame:
    ...
    df = df.astype({"Weight": "float64", "Price": "float64", "Chg": "float64"})
    df["% Chg"] = df["% Chg"].apply(lambda string: string[1:-2]).astype("float64")
...
def lambda_handler(event, context):
    ...
    return {
        "statusCode": 200,
        "body": json.dumps(
            f"Successfully uploaded SP500 csv file to bucket {S3_BUCKET}"
        ),
    }
